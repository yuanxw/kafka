...
class KafkaController(val config: KafkaConfig, zkUtils: ZkUtils, val brokerState: BrokerState, time: Time, metrics: Metrics, threadNamePrefix: Option[String] = None) extends Logging with KafkaMetricsGroup {
  ...
  def onControllerFailover() {
    if(isRunning) {
      ...
      // 尝试读取来自zookeeper的/controller_epoch节点，如果节点存在，并更新controllerContext的epoch
      readControllerEpochFromZookeeper()
      // 增加/controller_epoch节点
   ...
   * elector
   * 启动 Kafka 服务器的控制器模块时调用。这并一定当前代理是控制器。它只是在zookeeper注册会话过期侦听器并启动控制器领导者选举器
   */
  def startup() = {
    inLock(controllerContext.controllerLock) {
  ...
  def incrementControllerEpoch(zkClient: ZkClient) = {
    try {
      // newControllerEpoch = 当前控制器的epoch版本号加1
      var newControllerEpoch = controllerContext.epoch + 1
      // 更新控制器的epoch版本号，如果节点不存在，则在catch中创建节点
      ...
      case _: ZkNoNodeException =>
        ...
        try {
          // 创建/controller_epoch持久化节点，初始值为1
  ...
  private def registerSessionExpirationListener() = {
    // 在zookeeper中注册事件监听器，监听控制器的会话过期事件
  ...
  private def readControllerEpochFromZookeeper() {
    ...
    // 判断zookeeper中是否存在控制器的/controller_epoch节点，如果存在，则读取controller_epoch节点的值。
